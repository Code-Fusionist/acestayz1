      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {destinations.map((destination, index) => (
            <motion.div
              ...
              className="group flex flex-col items-center text-center p-6 bg-white rounded-xl shadow-sm hover:shadow-md transition duration-300 cursor-pointer hover:scale-105"
            >
              <MapPin className="w-8 h-8 text-ace-dark mb-2 group-hover:text-ace-gold transition-colors" />
              <h3 className="text-lg font-semibold text-ace-dark group-hover:text-ace-gold">{destination.name}</h3>
              <div className="flex items-center space-x-1 mt-2">
                <Star className="w-4 h-4 text-yellow-500 fill-yellow-500" />
                <span className="text-sm text-gray-600">{destination.rating}</span>
              </div>
              <p className="text-sm text-gray-500 mt-1">{destination.properties}</p>
            </motion.div>
          ))}
        </div>
        ...
        >
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="bg-ace-dark text-white px-4 py-2 rounded-full text-lg font-medium hover:bg-ace-gold transition-colors duration-300 shadow-xl"
          >
